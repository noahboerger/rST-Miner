...
/* harmony import */ var _eventlog_event__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../eventlog/event */ 9502);
/* harmony import */ var _eventlog_trace__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ../eventlog/trace */ 5997);
...
class LogParser {
  ...
  parseTraces(headers, eventLines) {
    ...
    asTable.forEach(eventLineSplit => {
      ...
      const eventLogAttributes = headers.filter(header => ![this._caseIdElement, this._activityElement].includes(header)).filter(header => headers.indexOf(header) < eventLineSplit.length).filter(header => eventLineSplit[headers.indexOf(header)] !== this._undefinedValue).map(header => LogParser.eventLogAttributeOf(header, eventLineSplit[headers.indexOf(header)]));
      ...
      (_a = dictCaseIdentifierToTrace.get(caseId)) === null || _a === void 0 ? void 0 : _a.events.push(new _eventlog_event__WEBPACK_IMPORTED_MODULE_1__.Event(eventLogAttributes, activity));
    });
  ...
  static eventLogAttributeOf(key, value) {
    switch (value) {
      case 'true':
        return new _eventlog_eventlog_attribute__WEBPACK_IMPORTED_MODULE_3__.BooleanAttribute(true, key);

      case 'false':
        return new _eventlog_eventlog_attribute__WEBPACK_IMPORTED_MODULE_3__.BooleanAttribute(false, key);
    }
    ...
    if (value.includes('T') || value.includes(':') || value.includes('-')) {
      ...
      if (!isNaN(timestamp)) {
        return new _eventlog_eventlog_attribute__WEBPACK_IMPORTED_MODULE_3__.DateAttribute(new Date(timestamp), key);
      }
    }
    ...
    if (value.includes('.') || value.includes(',')) {
      ...
      if (!isNaN(asFloat)) {
        return new _eventlog_eventlog_attribute__WEBPACK_IMPORTED_MODULE_3__.FloatAttribute(asFloat, key);
      }
    }
    ...
    if (!isNaN(asInt)) {
      return new _eventlog_eventlog_attribute__WEBPACK_IMPORTED_MODULE_3__.IntAttribute(asInt, key);
    }
...
/* harmony import */ var _classes_eventlog_eventlog__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../classes/eventlog/eventlog */ 5776);
/* harmony import */ var _classes_parser_logParser__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../classes/parser/logParser */ 949);
...
onmessage = function (data) {
  const parser = new _classes_parser_logParser__WEBPACK_IMPORTED_MODULE_1__.LogParser();
  const result = parser.parse(data.data);
  const serializer = new typedjson__WEBPACK_IMPORTED_MODULE_2__.TypedJSON(_classes_eventlog_eventlog__WEBPACK_IMPORTED_MODULE_0__.Eventlog);
...
/******/ 		var __webpack_exports__ = __webpack_require__.O(undefined, [180], () => (__webpack_require__(7722)))
