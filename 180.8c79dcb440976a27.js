...
class NoneOracle extends _trace_cleaner__WEBPACK_IMPORTED_MODULE_1__.TraceCleaner {
  determineConcurrency(eventlog) {
    return _models_concurrency_concurrency_relation__WEBPACK_IMPORTED_MODULE_0__.ConcurrencyRelation.noConcurrency(); // TODO ggf um dinstinguish same events ergänzen
  }
...
class LpoFirePlaceValidator extends _lpo_flow_place_validator__WEBPACK_IMPORTED_MODULE_1__.LpoFlowPlaceValidator {
  ...
  validate(toBeCheckedNet, toBeValidatedPlaceId) {
    ...
    this.fireForwards(places, queue, validPlaces, complexPlaces); // valid place
    ...
    const backwardsComplexPlaces = this.newBoolArray(places, false); // Is the final marking > 0 ?
...
/* harmony import */ var _petri_net_transformation_implicit_place_identifier__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ../petri-net/transformation/implicit-place-identifier */ 1485);
/* harmony import */ var _petri_net_validation_lpo_fire_place_validator__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ../petri-net/validation/lpo-fire-place-validator */ 6595);
...
class RstMiner {
  ...
  mine(eventlog) {
    ...
    const implicitPlaceIdentifier = new _petri_net_transformation_implicit_place_identifier__WEBPACK_IMPORTED_MODULE_6__.ImplicitPlaceIdentifier([...allTransitionActivities], partialOrderNetsWithContainedTraces.flatMap(partialOrderNetsWithContainedTraces => partialOrderNetsWithContainedTraces.containedTraces));
    let petriNet = this.createFlowerModel(allTransitionActivities);
    const lpoFirePlaceValidators = partialOrders.map(partialOrder => new _petri_net_validation_lpo_fire_place_validator__WEBPACK_IMPORTED_MODULE_7__.LpoFirePlaceValidator(partialOrder));

    const terminationConditionReachedFct = this._minerSettings.terminationCondition.toIsTerminationConditionReachedFunction();

    this._counterTestedPlacesLastRun = this._randomPlaceGenerator.init(petriNet, partialOrders);

    while (!terminationConditionReachedFct(petriNet)) {
      const clonedPetriNet = petriNet.clone();

      const addedPlace = this._randomPlaceGenerator.insertRandomPlace('p' + this._counterTestedPlacesLastRun++, clonedPetriNet); // TODO use template place instead
      // TODO hauptalgorithmus mit würfeln Testen und optimieren


      if (!RstMiner.isGeneratedPlaceValid(addedPlace, clonedPetriNet, lpoFirePlaceValidators)) {
        // TODO anpassen mit filter für noise reduction
        continue;
      }

      petriNet = clonedPetriNet;
      this._counterTestedPlacesLastRun = this.removeImplicitPlacesForAndIncreaseCounter(implicitPlaceIdentifier, addedPlace, petriNet, this._counterTestedPlacesLastRun);
  ...
  removeImplicitPlacesForAndIncreaseCounter(implicitPlaceIdentifier, addedPlace, petriNet, counter) {
    implicitPlaceIdentifier.calculateImplicitPlacesFor(addedPlace, petriNet).filter(implicitResult => RstMiner.checkSubstitutePlaceIsValidOrUndefined(implicitResult.substitutePlace)).forEach(implicitResult => {
  ...
  static checkSubstitutePlaceIsValidOrUndefined(substitutePlace) {
  ...
  static isGeneratedPlaceValid(addedPlace, clonedPetriNet, lpoFirePlaceValidators) {
    for (const lpoFirePlaceValidator of lpoFirePlaceValidators) {
      const validationResultForPlaceAndOrder = lpoFirePlaceValidator.validate(clonedPetriNet.clone(), addedPlace.id);
  ...
  get counterTestedPlacesLastRun() {
...
LoopBasedTerminationConfig.DEFAULT_ITERATIONS = 100000;
...
onmessage = function (data) {
  ...
  console.log("rST-Miner: Evaluated " + rstMiner.counterTestedPlacesLastRun + " places");
