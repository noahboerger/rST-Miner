...
class PrimitivePlaceGenerator {
  ...
  insertRandomPlace(id, petriNet) {
    ...
    Array.from(petriNet.getTransitions()).filter(transition => Math.random() < this._probability).map(transition => new _models_petri_net_arc__WEBPACK_IMPORTED_MODULE_1__.Arc('i' + id + transition.label, transition, newPlace)).forEach(arc => petriNet.addArc(arc));
    Array.from(petriNet.getTransitions()).filter(transition => Math.random() < this._probability).map(transition => new _models_petri_net_arc__WEBPACK_IMPORTED_MODULE_1__.Arc('o' + id + transition.label, newPlace, transition)).forEach(arc => petriNet.addArc(arc));
    return newPlace;
  }
